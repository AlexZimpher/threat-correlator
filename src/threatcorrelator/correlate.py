# Functions for log correlation and enrichment in ThreatCorrelator.
# Each function is robust and clearly explained for maintainability.
...
from defusedxml import ElementTree as ET  # Secure XML parsing
...
def extract_indicators_from_log(log_file_path: Path) -> dict[str, int]:
    ...
    Handles common log formats and flags anomalies.
    """
    freq: dict[str, int] = {}
...
def rate_threat(
    ...
    if max_age_days > 0 and last_seen:
        try:
            ...
            logger.warning("Error parsing last_seen date: %s", exc)
    if confidence >= 85:
        return "High"
    elif confidence >= 50:
        return "Medium"
...
def lookup_ioc(session, indicator: str) -> Optional[IOC]:
    ...
    if not ioc:
        ioc = (
            session.query(IOC)
            .filter(IOC.indicator == indicator, IOC.type == "domain")
            .first()
        )
    return ioc
...
def format_correlation_result(ioc: IOC, count: int) -> dict:
    ...
    return {
        ...
        "severity": rate_threat(
            confidence or 0, count, last_seen, freq_thresh, max_age_days, ioc_type_str
        ),
...
def correlate_logs(
    log_file_path: Path, geoip_db_path: Optional[str] = None, session=None
) -> list[dict]:
    """
