...
def test_cli_count(tmp_path):
    ...
    assert "IOCs stored" in result.output
...
def test_cli_export(tmp_path):
    ...
    ioc = IOC(indicator="9.9.9.9", confidence=99, country="US", last_seen=None, usage="test", source="test", type="ip")
    session.add(ioc)
    session.commit()
    # Create log file
    log_path = tmp_path / "log.jsonl"
    ...
    result = runner.invoke(cli, ["export", str(log_path), "-o", str(output_path), "-c", "90"])
    assert result.exit_code == 0
