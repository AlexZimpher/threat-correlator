...
def enrich_ioc_row(ioc):
    if not getattr(ioc, 'severity', None):
        if getattr(ioc, 'confidence', 0) >= 85:
            severity = "High"
        elif getattr(ioc, 'confidence', 0) >= 50:
            severity = "Medium"
        else:
            severity = "Low"
    else:
...
def main():
    ...
    if iocs:
        ...
        with tab1:
            ...
            conf_min, conf_max = st.sidebar.slider("Confidence Range", 0, 100, (0, 100), key="conf")
            country_filter = st.sidebar.multiselect("Country", options=sorted(df["Country"].dropna().unique()), key="country")
            filtered = df[(df["Confidence"] >= conf_min) & (df["Confidence"] <= conf_max)]
            if country_filter:
                filtered = filtered[filtered["Country"].isin(country_filter)]
            st.metric("Total IOCs", len(filtered))
            st.dataframe(filtered, use_container_width=True, height=400)
            st.download_button("⬇️ Download CSV", filtered.to_csv(index=False), file_name="ioc_results.csv")
            if st.button("Export as PDF"):
                pdf_bytes = generate_pdf(filtered)
                st.download_button("⬇️ Download PDF", pdf_bytes, file_name="ioc_results.pdf")
        with tab3:
            ...
            if "MITRE Technique" in filtered:
                ...
                fig = px.bar(tech_counts, x="MITRE Technique", y="Count", title="MITRE ATT&CK Technique Breakdown")
                st.plotly_chart(fig, use_container_width=True)
    else:
    ...
    with tab2:
        ...
        st.caption("Upload a log file to scan for known threats. Supported: Suricata-style JSON, Apache, Windows XML.")
